...
def get_data():
    try:
        # 店舗データの読み込み（2行目を見出しとして認識）
        df = pd.read_csv(S2_URL, header=1, engine='c')
        df.columns = df.columns.str.strip()
        # カラム名のマッピング（日本語からプログラム用へ）
...
<body class="pb-20">
    <nav class="bg-orange-600 text-white p-5 text-center font-bold shadow-lg text-lg tracking-widest">能登復興支援ポータル</nav>
    ...
    <div class="max-w-md mx-auto px-4">
        ...
        <div class="bg-white p-8 rounded-3xl shadow-xl border-t-8 border-slate-800 text-center">
            ...
            <p class="text-sm text-slate-500 mb-8 leading-relaxed">右下の「＋」ボタンからお店を登録してください。<br>入力完了後に発行される内容を管理者に送ると、<br>サイトへ反映されます。</p>
        </div>
        {% elif shop %}
        <div class="bg-white rounded-3xl shadow-xl overflow-hidden border-t-8 border-orange-500 mb-8">
            ...
            <div class="p-8">
                ...
                <div class="bg-orange-50 p-4 rounded-xl text-sm italic mb-8 text-slate-600">「{{ shop.msg }}」</div>
                <a href="{{ shop.ec }}" target="_blank" class="block w-full py-4 bg-orange-500 text-white rounded-2xl font-bold text-center shadow-lg mb-10">🛒 通販サイトでお買い物</a>
                ...
                <div class="pt-6 border-t border-slate-100">
                    ...
                    {% if comments %}
                        ...
                        {% endfor %}
                    {% else %}
                        <p class="text-xs text-slate-400 italic text-center py-4">まだ応援メッセージはありません。<br>最初の一件を募集中です！</p>
                    {% endif %}
                </div>
            </div>
        </div>
        ...
        {% else %}
        <div class="space-y-4">
            <p class="text-center text-slate-400 text-[10px] font-bold tracking-widest uppercase">復興を目指すお店一覧</p>
            {% for i in all %}
    ...
    {% if r == 'o' %}
    <button onclick="document.getElementById('m').style.display='block'" class="fixed bottom-6 right-6 w-16 h-16 bg-slate-800 text-white rounded-full shadow-2xl text-4xl font-light active:scale-90 transition">+</button>
    <div id="m" class="modal">
        <div class="m-content shadow-2xl">
            ...
            <form onsubmit="cp(event)" class="space-y-4 pb-8">
                <input type="text" id="fn" placeholder="お店の名前 *" required class="w-full p-4 rounded-2xl bg-slate-50 border outline-none focus:border-orange-500">
                ...
                <textarea id="fm" placeholder="店主からのメッセージ" class="w-full p-4 rounded-2xl bg-slate-50 border h-28 outline-none focus:border-orange-500"></textarea>
                <input type="url" id="fi" placeholder="写真URL（SNS写真のリンクなど）" class="w-full p-4 rounded-2xl bg-slate-50 border outline-none focus:border-orange-500">
                <button type="submit" class="w-full py-5 bg-slate-800 text-white rounded-2xl font-bold text-lg shadow-xl active:scale-95 transition">申請内容をコピーする</button>
            </form>
        </div>
    </div>
    <script>
        function cp(e){
            ...
            alert('コピーしました！この内容を管理者に送ってください。承認後、サイトに表示されます。');
...
def shop(sid):
    ...
    # Sheet3のデータから、このお店のIDに一致するコメントのみを抽出
