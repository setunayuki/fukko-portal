...
def index():
    ...
    if df is None or df.empty:
        return "データの読み込みに失敗しました。スプレッドシートの共有設定が「編集者」になっているか確認してください。"
    
    # 確実に見つけられる「名前」を使ってデータを取得します
    row = df.iloc[0]
    shop = {
        ...
        "recommendation": row.get('recommendation', row.get('recommendatior', 'なし')),
